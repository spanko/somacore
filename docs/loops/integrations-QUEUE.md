...
## M1 — MFP CSV upload path — `done f3fad39`
...
- 2026-07-12 (M1): MFP's export ZIP is documented (help-center) as three CSVs with meal-level nutrition summarized by meal + food notes — exact column names are NOT published. The parser matches headers case-insensitively with unit-suffix stripping ("Fat (g)"→fat) and falls back to any CSV with Date+Meal columns; food_items comes from a Foods/Note column when present. Verify against Adam's real export before the flag flips.
- 2026-07-12 (M1): no ADR 0011 ingestion trace root for CSV uploads — the contract keys on external_connection_id, which a first-party file upload doesn't have. Revisit if the trace contract grows a connectionless variant.
- 2026-07-12 (loop complete): all 8 items done in one session, 8 evaluator PASSes, 2 evaluator invocations needed a resume-nudge to emit their verdict, 1 pre-existing test updated for an intentional schema change (S6 source→sources). Total evaluator invocations: 8 (incl. re-invocations of the same agent, well under the ~10 cap).
